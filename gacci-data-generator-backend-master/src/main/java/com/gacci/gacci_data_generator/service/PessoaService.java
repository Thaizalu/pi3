...
public class PessoaService {
    ...
    public List<Pessoa> findAll() {
        return retiraInativo(repository.findAll());
    }

    public List<Pessoa> findByFilter(String filter) {
        return retiraInativo( repository.findByNomeContainingIgnoreCaseOrCpfContainingIgnoreCase(filter, filter) );
    }

    public List<Pessoa> retiraInativo( List<Pessoa> pessoas){
        return pessoas.stream().filter(Pessoa::getAtivo).toList();
    }
    ...
    private String montaDetalhes(Pessoa pessoa) {
        ...
        // --- Anotação do revisor marido da Thaiza puto da vida:
        //
        // Você ta de brincadeira com minha face ne
        // Montar HTML no backend?
        // Você é a vergonha da profission
        // Cala a boca!!!!!!
        // Retorna os dados e o FRONT se encarrega disso.
